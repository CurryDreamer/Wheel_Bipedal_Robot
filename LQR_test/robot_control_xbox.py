        ...
        while viewer.is_running():
            ...
            for event in pygame.event.get():
                ...
                # 处理手柄按键按下事件 (单次点击生效)
                    ...
                    # 获取该事件对应的按钮编号
                    if event.button == 4:  # 通常对应 LB (Left Bumper)
                        target_length = float(np.clip(target_length - 0.005, 0.10, 0.14))
                        print(f"手柄按键 4 被按下，目标腿长减少，当前腿长为: {target_length:.3f}")
                    elif event.button == 5: # 通常对应 RB (Right Bumper)
                        target_length = float(np.clip(target_length + 0.005, 0.10, 0.14))
                        print(f"手柄按键 5 被按下，目标腿长增加，当前腿长为: {target_length:.3f}")
            ...
            if joystick is not None:
                ...
                # 轴 1：控制 target_v，通常是左摇杆上下 (推上为负值，所以取负以使得推上为前进)
                axis_1 = joystick.get_axis(1)
                if abs(axis_1) > deadzone:
                    target_v = -axis_1 * 0.6  # 最大速度映射到 0.6
                    
                # 轴 3：控制 target_w，通常是右摇杆左右 (向右推为正值，向右转时角速度为负，所以取负)
                axis_3 = joystick.get_axis(3)
                if abs(axis_3) > deadzone:
                    target_w = -axis_3 * 2.0  # 最大角速度映射到 2.0
            ...
            # 线性速度爬坡 (可以分别设置加速和减速/刹车的加速度大小)
            accel_rate = 0.5  # 加速度 (m/s^2)
            decel_rate = 0.8  # 减速度 (m/s^2)，通常刹车需要更快

            # 判断当前是加速还是减速
            # 如果目标速度绝对值大于当前速度绝对值，且方向相同（或者当前速度为0），则视为加速
            if abs(target_v) > abs(current_v) and target_v * current_v >= 0:
                ramp_rate = accel_rate
            else:
                ramp_rate = decel_rate

            max_step_size = abs(dt * ramp_rate)
            ...
            left_F_0 = (4 / math.cos(five_links_left.theta) if math.cos(five_links_left.theta) != 0 else 0) + left_F_0_ctrl
            ...
            right_F_0 = (4 / math.cos(five_links_right.theta) if math.cos(five_links_right.theta) != 0 else 0) + right_F_0_ctrl
